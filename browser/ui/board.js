...
//   getBottomPlayer() -> player index rendered at the bottom (orientation only)
//   getActor()        -> player whose pieces are interactive right now (the one
//                        to move); == bottom seat except local pass & play, where
//                        the board doesn't flip so the actor may be at the top
...
export function createBoardView(mount, ctx) {
  ...
  function update(stateOverride) {
    ...
    if (!state) return;
    const bottom = ctx.getBottomPlayer();          // orientation: this seat at the bottom
    const actor = ctx.getActor ? ctx.getActor() : bottom; // whose pieces are live now
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
      for (let c = 0; c < BOARD_SIZE; c++) {
        ...
        const src = act && t && t.p === actor ? ' data-src="cell"' : '';
    ...
    grid.innerHTML = g;
    // Each reserve is interactive only when its owner is the one to move — so in
    // pass & play the top player can act on their turn without the board flipping.
    reserveTop.innerHTML = reserveHTML(state, 1 - bottom, (1 - bottom) === actor);
    reserveBottom.innerHTML = reserveHTML(state, bottom, bottom === actor);
  }
